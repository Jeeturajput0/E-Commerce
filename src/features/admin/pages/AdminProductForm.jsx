...
const AdminProductForm = () => {
    ...
    (async () => {
      try {
        const [category, brand, size, color] = await Promise.all(
          ["category", "brand", "size", "color"].map((x) => api(`/admin/${x}`).catch(() => []))
        );
        setLists({
  ...
  return (
    <div className="mx-auto max-w-3xl space-y-6">
      ...
      <div>
        <p className="text-xs font-semibold uppercase tracking-[.18em] text-primary-600">Catalog</p>
        <h2 className="mt-1 text-3xl font-bold">{id ? "Edit Product" : "Add Product"}</h2>
        <p className="mt-2 text-slate-500">Admin products are published immediately after saving.</p>
      </div>
      <Card className="border border-slate-200 p-6">
        <form onSubmit={save} className="grid gap-4">
          ...
          <div className="flex justify-end gap-3 border-t pt-5">
            <Button variant="ghost" onClick={() => navigate("/admin/dashboard/products")}>Cancel</Button>
            <Button type="submit" disabled={saving}>{saving ? "Saving..." : id ? "Save changes" : "Add Product"}</Button>
          </div>
        </form>
      </Card>
    </div>
