...
import { vendorLinks as rawVendorLinks, headerIconButton } from "../constants";
...
export const vendorLinks = rawVendorLinks.map((link) => ({
  ...link,
    ...
    <div className="flex flex-col justify-between gap-4 lg:flex-row lg:items-center">
      ...
      <div className="flex flex-1 flex-wrap items-center justify-end gap-2">
        <label className="flex min-w-[230px] flex-1 items-center gap-2 rounded-xl border border-slate-200 bg-slate-50 px-3 py-2 text-sm text-slate-500 dark:border-slate-700 dark:bg-slate-800 dark:text-slate-300">
          ...
          <input className="w-full bg-transparent outline-none" placeholder="Search products, orders, earnings..." />
        </label>
        <button className={headerIconButton}><Settings2 className="h-4 w-4" /></button>
        <button className={headerIconButton}><Bell className="h-4 w-4" /></button>
        <button className={headerIconButton} onClick={toggleTheme} title="Toggle black theme">
        ...
        <div className="relative">
          ...
          >
            ...
            <div className="text-xs">
              <p className="font-semibold leading-none">{currentUser?.name || "Vendor"}</p>
              <p className="mt-1 text-slate-500 dark:text-slate-300">Store Manager</p>
            </div>
...
export default VendorHeaderContent;
